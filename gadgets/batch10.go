...
type RealQuizGame struct {
	...
	Finished  bool
	mu        sync.RWMutex
	OnComplete func(score, total int)
}
...
func NewRealQuizGame(id string) *RealQuizGame {
	return &RealQuizGame{
		Base: *NewBase(id),
		Questions: []QuizQuestion{
			{Text: "What is the capital of France?", Options: []string{"London", "Berlin", "Paris", "Madrid"}, Answer: 2},
			{Text: "What is 7 * 8?", Options: []string{"54", "56", "48", "63"}, Answer: 1},
			{Text: "Which planet is closest to the Sun?", Options: []string{"Venus", "Earth", "Mercury", "Mars"}, Answer: 2},
			{Text: "What is the largest ocean?", Options: []string{"Atlantic", "Indian", "Arctic", "Pacific"}, Answer: 3},
			{Text: "Who painted the Mona Lisa?", Options: []string{"Van Gogh", "Da Vinci", "Picasso", "Monet"}, Answer: 1},
		},
	}
}

func (g *RealQuizGame) Answer(idx int) {
	g.mu.Lock()
	defer g.mu.Unlock()

	if g.Current >= len(g.Questions) || g.Finished {
		return
	}

	g.Answered++
	if idx == g.Questions[g.Current].Answer {
		g.Score++
	}
	...
	if g.Current >= len(g.Questions) {
		g.Finished = true
		if g.OnComplete != nil {
			g.OnComplete(g.Score, len(g.Questions))
		}
	}
}

func (g *RealQuizGame) Render(ctx *mofu.RenderContext) {
	g.mu.RLock()
	defer g.mu.RUnlock()
	...
	ctx.Renderer.WriteString(fmt.Sprintf(" Quiz  Score: %d/%d", g.Score, g.Answered), r.X, y, mofu.Hex("ff69b4"), mofu.ColorBlack, mofu.AttrBold)
	y++
	ctx.Renderer.WriteString(strings.Repeat("─", r.Width-2), r.X+1, y, mofu.Hex("444444"), mofu.ColorBlack, 0)
	y++

	if g.Finished {
		pct := float64(g.Score) / float64(len(g.Questions)) * 100
		ctx.Renderer.WriteString(fmt.Sprintf(" Final Score: %d/%d (%.0f%%)", g.Score, len(g.Questions), pct), r.X, y, mofu.Hex("a6e3a1"), mofu.ColorBlack, mofu.AttrBold)
		return
	}
	...
	ctx.Renderer.WriteString(fmt.Sprintf(" Q%d: %s", g.Current+1, q.Text), r.X, y, mofu.Hex("cdd6f4"), mofu.ColorBlack, mofu.AttrBold)
	y++
	y++
	...
	for i, opt := range q.Options {
		...
		ctx.Renderer.WriteString(fmt.Sprintf("%s%s. %s", prefix, string(rune('a'+i)), opt), r.X, y, color, mofu.ColorBlack, 0)
		y++
	}
}

func (g *RealQuizGame) HandleEvent(e mofu.Event) mofu.Cmd {
	if e.Type != mofu.EventKeyPress {
		return nil
	}
	ke := e.Data.(mofu.KeyEvent)
